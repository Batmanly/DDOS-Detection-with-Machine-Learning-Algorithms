...
class SimpleMonitor13(switch.SimpleSwitch13):
    ...
    def flow_training(self):
        ...
        # drop some columns
        # flow_dataset.drop(['flow_id','idle_timeout','hard_timeout','timestamp','ip_src','tp_src','ip_dst','tp_dst','flags','datapath_id'], axis=1, inplace=True)
        ...
        self.X_train = self.scaler.fit_transform(X_flow_train)
        self.X_test = self.scaler.transform(X_flow_test)

        if 'train' == get_command_variables():

            # Training the SVM model on the Training set
            # check if model exist
            if os.path.exists('../ml/TrainedModels/svm_model.pkl'):
                self.logger.info("Flow Model exist")
                self.flow_model = joblib.load('../ml/TrainedModels/svm_model.pkl')
            else:
                self.logger.info("Flow Model not exist")
                self.logger.info("Flow Model training")
                self.flow_model = classifier.fit(self.X_train, y_flow_train)
                joblib.dump(self.flow_model, '../ml/TrainedModels/svm_model.pkl')

            # Predicting the Test set results
            y_flow_pred = self.flow_model.predict(self.X_test)
    ...
    def flow_predict(self):
        try:
            ...
            # predict_flow_dataset.drop(['flow_id','idle_timeout','hard_timeout','timestamp','ip_src','tp_src','ip_dst','tp_dst','flags','datapath_id'], axis=1, inplace=True)

            X_predict_flow = predict_flow_dataset.iloc[:, :].values
            X_predict_flow = X_predict_flow.astype('float64')

            # X_predict_flow = np.array(X_predict_flow)
            # Feature scaling (or standardization)

            self.X_test = self.scaler.transform(X_predict_flow)
            ...
            self.logger.info(
                "------------------------------------------------------------------------------")
            print(legitimate_trafic / len(y_flow_pred) * 100)
            ...
            self.logger.info("Flow Predicting Error ...")
            self.logger.info(e)
