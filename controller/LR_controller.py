...
class SimpleMonitor13(switch.SimpleSwitch13):
    ...
    def flow_training(self):
        ...
        if 'train' == get_command_variables():
            ...
            if os.path.exists('../ml/TrainedModels/lr_model.pkl'):
                self.logger.info("Flow Model exist")
                self.flow_model = joblib.load('../ml/TrainedModels/lr_model.pkl')
            else:
                self.logger.info("Flow Model not exist")
                self.logger.info("Flow Model training")
                self.flow_model = classifier.fit(X_flow_train, y_flow_train)
                joblib.dump(self.flow_model, '../ml/TrainedModels/lr_model.pkl')
